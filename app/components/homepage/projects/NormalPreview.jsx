    ...
    return (
        <div className="overflow-hidden border-t-[2px] border-indigo-900 px-4 lg:px-8 py-2 lg:py-4">
            <div className=" text-xs md:text-sm lg:text-base flex flex-col items-center">
                ...
                <div className={`${!project.image && "pt-8"} flex flex-col pt-3`}>
                    <h5 className={`${!project.image && "pb-8"} font-mono text-base lg:text-2xl text-pink-400 text-center pb-2`}><b>{project.subTitle}</b></h5>
                ...
                <div className="tools-area gap-1 pt-3">
                    {
                        project.tools.map((tool, index) => {
                            return (
                                <span key={index} className="text-xs md:text-sm lg:text-sm text-[#16f2b3] bg-[#241c59] px-2 py-1 rounded-md">{tool}</span>
                            )
                        })
                    }
                </div>

                <Link href={project.demo} target='_blank' className={`mt-4 ${!project.image && "2xl:mt-10 lg:mt-10 mt-10"}  lg:mt-2 2xl:mt-4 bg-gradient-to-r to-pink-500 from-violet-600 p-[1px] rounded-full transition-all duration-300 hover:from-pink-500 hover:to-violet-600`}>
